...
void main() {
  ...
  test('Offline Flow: Open Shift -> Create Bill -> Sync Queue Verification',
      () async {
    // 1. Open Shift
    ...
    final shiftSync =
        syncItems.firstWhere((i) => i.targetCollection == 'shifts');
    ...
    // A. Stock Deducted
    final tank = await (db.select(db.tanks)
          ..where((t) => t.tankId.equals('t1')))
        .getSingle();
    expect(tank.currentStock, 4990.0); // 5000 - 10

    // B. Nozzle Reading Updated
    final updatedNozzle = await (db.select(db.nozzles)
          ..where((t) => t.nozzleId.equals('n1')))
        .getSingle();
    ...
    final cashAcc = await (db.select(db.ledgerAccounts)
          ..where((t) => t.id.equals('cash_acc')))
        .getSingle();
    final salesAcc = await (db.select(db.ledgerAccounts)
          ..where((t) => t.id.equals('sales_acc')))
        .getSingle();
    ...
    final billSync = syncItems.firstWhere((i) => i.targetCollection == 'bills');
    expect(billSync.operationType, 'create');

    final tankSync = syncItems.firstWhere((i) => i.targetCollection == 'tanks');
    expect(tankSync.operationType, 'update');

    final nozzleSync =
        syncItems.firstWhere((i) => i.targetCollection == 'nozzles');
    expect(nozzleSync.operationType, 'update');

    final stockSync =
        syncItems.firstWhere((i) => i.targetCollection == 'stock_movements');
...
Future<void> _seedData(AppDatabase db) async {
  // Tanks
  await db.into(db.tanks).insert(TanksCompanion.insert(
        tankId: 't1',
        ownerId: 'test_owner_1',
        name: 'Tank 1',
        fuelTypeId: 'petrol',
  ...
  // Dispensers
  await db.into(db.dispensers).insert(DispensersCompanion.insert(
        id: 'd1',
        ownerId: 'test_owner_1',
        name: 'Dispenser 1',
  ...
  // Nozzles
  await db.into(db.nozzles).insert(NozzlesCompanion.insert(
        nozzleId: 'n1',
        ownerId: 'test_owner_1',
        dispenserId: 'd1',
        name: 'Nozzle 1',
  ...
  await db.into(db.ledgerAccounts).insert(LedgerAccountsCompanion.insert(
        id: 'cash_acc',
        userId: 'test_owner_1',
        name: 'Cash',
        type: 'ASSET',
