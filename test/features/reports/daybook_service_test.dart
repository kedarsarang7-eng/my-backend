...
void main() {
  ...
  test('computeDaySummary aggregates all transactions correctly', () async {
    ...
    await db.into(db.bills).insert(BillsCompanion(
          id: Value('b1'), userId: Value(businessId),
          invoiceNumber: Value('INV-1'), itemsJson: Value('[]'),
          billDate: Value(date), grandTotal: Value(1000),
          paymentMode: Value('Cash'), cashPaid: Value(1000),
          createdAt: Value(date), updatedAt: Value(date),
    ...
    await db.into(db.purchaseOrders).insert(PurchaseOrdersCompanion(
          id: Value('po1'),
          userId: Value(businessId),
    ...
    // Verify Sync Queue
    final syncItem = await (db.select(db.syncQueue)
          ..where((t) => t.targetCollection.equals('day_book')))
        .getSingleOrNull();
