...
void main() {
  ...
  group('Hard Business Type Isolation Verification', () {
    ...
    test('Grocery Shop CANNOT use IMEI', () {
      expect(FeatureResolver.canAccess('grocery', BusinessCapability.useIMEI),
          isFalse);
    });
    ...
    test('Pharmacy Shop CAN use Prescription but NOT IMEI', () {
      ...
      expect(
          FeatureResolver.canAccess(
              pharmacy, BusinessCapability.usePrescription),
          isTrue);
      expect(FeatureResolver.canAccess(pharmacy, BusinessCapability.useIMEI),
          isFalse);
    });

    test('Electronics Shop CAN use IMEI but NOT Prescription', () {
      final electronics = 'electronics';
      expect(FeatureResolver.canAccess(electronics, BusinessCapability.useIMEI),
          isTrue);
      expect(
          FeatureResolver.canAccess(
              electronics, BusinessCapability.usePrescription),
          isFalse);
    });
    ...
    test('Business Type Lock: Cannot change type if data exists', () async {
      ...
      expect(
        ...
        throwsA(isA<Exception>().having(
            (e) => e.toString(), 'msg', contains('Business Type is LOCKED'))),
      );
    });
