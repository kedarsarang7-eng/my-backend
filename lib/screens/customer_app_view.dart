...
class CustomerAppView extends StatefulWidget {
  ...
  const CustomerAppView(
      {required this.phoneNumber, this.isOwnerMode = false, super.key});
                      ...
                      child: Column(
                          ...
                          children: [
                            Text('Total Pending Due',
                                style:
                                    TextStyle(color: FuturisticColors.error)),
      ...
      builder: (context, snapshot) {
        ...
        return SingleChildScrollView(
          ...
          child:
            ...
            const SizedBox(height: 12),
            _infoCard('Total Spent', '₹${totalSpent.toStringAsFixed(2)}',
                Icons.shopping_cart, Colors.blue),
            const SizedBox(height: 8),
            _infoCard('Total Paid', '₹${totalPaid.toStringAsFixed(2)}',
                Icons.check_circle, FuturisticColors.success),
            const SizedBox(height: 8),
            Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
                ...
                child: Row(children: [
                  Icon(totalDue > 0 ? Icons.pending : Icons.check_circle,
                      color: totalDue > 0
                          ? FuturisticColors.error
                          : FuturisticColors.success),
                  const SizedBox(width: 12),
                  Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(totalDue > 0 ? 'Total Pending' : 'All Bills Paid',
                            style: TextStyle(
                                color: totalDue > 0
                                    ? FuturisticColors.error
                                    : FuturisticColors.success,
                ...
                builder: (c, snap) {
                  ...
                  return Column(children: [
                    Row(children: [
                      Expanded(
                          child: DropdownButtonFormField<String>(
                              ...
                              onChanged: (vid) {
                                final product =
                                    products.firstWhere((v) => v.id == vid);
                    ...
                    DropdownButtonFormField<String>(
                        ...
                        items: const [
                          DropdownMenuItem(
                              value: 'none', child: Text('No payment now')),
                    ...
                    if (paymentMethod != 'none')
                      TextField(
                          ...
                          onChanged: (v) => payNow = double.tryParse(v) ?? 0.0),
                  ]);
                },
              ),
            ]),
          ...
          TextButton(
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                  content:
                      Text('Please contact the shop for UPI payment details')));
              Navigator.pop(ctx);
            },
    ...
    showDialog(
        ...
        builder: (dialogContext) => StreamBuilder<List<Customer>>(
            ...
            builder: (streamContext, snapshot) {
              ...
              Customer? customer;
              try {
                customer = snapshot.data!.firstWhere((c) =>
                    c.phone == widget.phoneNumber ||
                    c.name == widget.phoneNumber);
              } catch (_) {}
                  ...
                  ElevatedButton(
                      onPressed: () async {
                        ...
                        if (newDueAmount >= 0) {
                          try {
                            final updated =
                                customer!.copyWith(totalDues: newDueAmount);
                            await _customersRepo.updateCustomer(updated,
                                userId: ownerId);
